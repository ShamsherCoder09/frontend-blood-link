...
function Header() {
  ...
  return (
    <> 
    <div className="px-2 flex justify-between items-center h-20    font-sans-">
      <div className=" text-red-500 font-bold text-lg">
        BloodLink {/* BLOODLINK Actual Logo Will Placed here */}
      </div>
      <ul className="hidden gap-6 items-center md:flex">
          {navItems.map((item) => <li key={item.id} className={`cursor-pointer ${item.className} ${navStyling}`}>{item.name}</li>)}
      </ul>
      ...
      <div className="md:hidden">
        ...
        {displayNav &&  <ul className="flex gap-6 absolute right-16 top-20 text-right flex-col">
          {navItems.map(item => <li className={`${navStyling}`} key={item.id}>
            {item.name}
          </li>)}
        </ul>
